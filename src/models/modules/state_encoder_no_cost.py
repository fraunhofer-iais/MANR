...
class SeqLSTM(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder = nn.LSTM(input_size=self.embedding_size-1, hidden_size=self.hidden_size, num_layers=self.num_layers,
                               batch_first=True, dropout=self.dropout_rate, bidirectional=True)   #embedding size -1 since no more cost
    ...
    def calc_embedding(self, dm_list, take_x_y):
        ...
        for agent_id in range(num_agents):
            ...
            for dm in dm_list:
                encoder_input.append([node_rep[:-1] for node_rep in dm.routes[agent_id][:]])  #exclude cost info
            ...
            for i, dm in enumerate(dm_list):
                while len(encoder_input[i]) < max_node_cnt:
                    if take_x_y:
                        ...
                        encoder_input[i].append([agent_depot_idx, agent_depot_idx])
            ...
            for dm_idx, dm in enumerate(dm_list):
                ...
                dm.encoder_outputs_no_cost[agent_id] = encoder_output[dm_idx][:num_nodes_in_cur_ag_route]
                # get one agent depot representation by averaging output of first and last depot
                cur_encoder_output = dm.encoder_outputs_no_cost[agent_id].clone()
                ...
                dm.encoder_outputs_no_cost[agent_id] = cur_encoder_output
